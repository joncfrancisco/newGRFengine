"""Tests for the pure-text NML builders."""
...
def test_switch_includes_a_default_only_when_given():
    ...
    with_default = nw.switch("FEAT_TRAINS", "SELF", "sw", "expr",
                             [("1", "a")], default="b")
...
def test_nml_wraps_the_header_as_a_comment_block():
    text = nw.NML(header="line one\nline two").text()
    assert text.startswith("/*")
    assert " * line one" in text and " * line two" in text


def test_nml_write_creates_directories(tmp_path):
    path = nw.NML().add("content").write(str(tmp_path / "nested" / "out.pnml"))
